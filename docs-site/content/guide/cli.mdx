...
Creates a `package.json` and `tsconfig.json` in the current directory with all
required compiler options pre-configured.

| Option | Default | Description |
|--------|---------|-------------|
| `--name <name>` | `my-parser` | npm package name for the scaffolded project |
...
These options are **required** — the generated parser uses native `#` private
fields (`target: ES2022`), is verified to compile cleanly under strict null checks
(`strict: true`, `noUncheckedIndexedAccess: true`), and needs the package exports
map (`moduleResolution: node16` or `bundler`).
